...
export class DaytonaSandbox implements SandboxBackendProtocol {
  ...
  async read(filePath: string, offset = 0, limit = 500): Promise<string> {
    try {
      ...
      const content =
        typeof buffer === "string" ? buffer : new TextDecoder().decode(buffer as Uint8Array)
  ...
  async write(filePath: string, content: string): Promise<WriteResult> {
    try {
      ...
      await sandbox.fs.uploadFile(new TextEncoder().encode(content), resolvedPath)
  ...
  async uploadFiles(files: Array<[string, Uint8Array]>): Promise<FileUploadResponse[]> {
    ...
    for (const [path, content] of files) {
      try {
        const sandbox = await this.ensureInitialized()
        const resolvedPath = this.resolvePath(path)
        await sandbox.fs.uploadFile(content, resolvedPath)
